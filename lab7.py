...
def run_lab():
    ...
    for n in sizes:
        ...
        print(f"Время Дейкстры: {end_t - start_t:.4f} сек")
        print(f"Итераций: {iters}")
        print(f"Кратчайшее расстояние до {n-1}: {dists[n-1]}")
        print(f"Путь (первые 5 узлов): {path[::-1][:5]}...")
  
    lab1 = GraphLab(1200)
    lab1.generate_graph()
    start_f = time.time()
    matrix = lab1.floyd_warshall()
    end_f = time.time()
    print(f"Время Флойд-Уоршелла: {end_f - start_f:.4f} сек")
